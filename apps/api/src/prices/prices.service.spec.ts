...
describe('PricesService', () => {
  ...
  describe('getModelPrices', () => {
    ...
    it('존재하지 않는 모델 조회 시 NotFoundException을 던진다', async () => {
      ...
      await expect(
        service.getModelPrices('nonexistent-id'),
  ...
  describe('getPopularModels', () => {
      ...
      const modelsWithPrices = [
        {
          ...mockModel,
          priceGuides: [
            { price: 850000, trend: PriceTrend.STABLE },
          ],
        },
      ];
    ...
    it('가격 가이드가 없는 모델은 null 가격과 STABLE 트렌드를 반환한다', async () => {
      const modelsWithoutPrices = [
        { ...mockModel, priceGuides: [] },
      ];
      ...
      const multiCategoryModels = [
        {
          ...mockModel,
          category: { ...mockCategory, type: 'SMARTPHONE' },
          priceGuides: [{ storage: '128GB', grade: ProductGrade.A, price: 850000 }],
        },
        {
          ...mockModel,
          id: 'model-uuid-3',
          name: 'iPad Pro',
          category: { id: 'cat-2', type: 'TABLET', name: '태블릿', sortOrder: 2 },
          priceGuides: [{ storage: '256GB', grade: ProductGrade.A, price: 1200000 }],
        },
      ];
  ...
  describe('calculateTrend', () => {
    ...
    it('최근 7일 이력을 조회한다', async () => {
      ...
      expect(prisma.priceHistory.findMany).toHaveBeenCalledWith({
        where: { modelId: 'model-uuid-1', storage: '128GB', grade: ProductGrade.A },
